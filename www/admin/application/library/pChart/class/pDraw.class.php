          ...
          foreach($PosArray as $Key => $Y) {
            if ( $DisplayValues && $Serie["Data"][$Key] != VOID ) {
              if ( $Serie["Data"][$Key] > 0 ) {
                ...
                $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset;
              }
    ...
    foreach($Data["Series"] as $SerieName => $Serie) {
      if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] ) {
        ...
        if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT ) {
          ...
          $Points = $PointsTop;
          for($i=count($PointsBottom)-2;$i>=0;$i-=2) {
            $Points[] = $PointsBottom[$i];
            $Points[] = $PointsBottom[$i+1];
          }
          $this->drawPolygon($Points,$Color);
          ...
          if ( $DisplayValues ) {
            foreach($Labels as $Key => $Value) {
              $this->drawText($Value[0],$Value[1],$Value[2],$Value[3]);
            }
          }
        } else {
          if ( $YZero < $this->GraphAreaX1+1 ) { $YZero = $this->GraphAreaX1+1; }
