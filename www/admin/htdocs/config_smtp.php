...
$replace = array(
        ...
        "__FORM_INPUTSMTPRELAY__" => $aform->textarea('relay_from_hosts', 22, 3, preg_replace('/[\s:]+/',"\n", htmlspecialchars($mta_in->getPref('relay_from_hosts'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'))),
        "__FORM_INPUTHOSTSACCESS__" => $aform->input('smtp_conn_access', 25, htmlspecialchars($mta_in->getPref('smtp_conn_access'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTHOSTREJECT__" => $aform->textarea('host_reject', 22, 3, htmlspecialchars($mta_in->getPref('host_reject'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTSENDERREJECT__" => $aform->textarea('sender_reject', 22, 3, htmlspecialchars($mta_in->getPref('sender_reject'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTVERIFYSENDER__" => $aform->checkbox('verify_sender', '1', htmlspecialchars($mta_in->getPref('verify_sender'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'), '', 1),
        "__FORM_INPUTENABLETLS__" => $aform->checkbox('use_incoming_tls', '1', htmlspecialchars($mta_in->getPref('use_incoming_tls'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'), '', 1),
        ...
        "__INPUT_GLOBALMAXMSGSIZE__" => $aform->input('global_msg_max_size', 6, htmlspecialchars($mta_in->getPref('global_msg_max_size'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        ...
        "__FORM_INPUTLDAPSERVER__" => $lform->input('ad_server', 20, htmlspecialchars($sysconf_->getPref('ad_server'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTBASEDN__" => $lform->input('ad_basedn', 20, htmlspecialchars($ldap_basedn_), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
        "__FORM_INPUTBINDDN__" => $lform->input('ad_binduser', 20, htmlspecialchars($ldap_binduser_), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
        "__FORM_INPUTBINDPASSWORD__" => $lform->password('ad_bindpass', 20, htmlspecialchars($ldap_bindpass_), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
        "__LDAPCALLOUT_SAVE_STATUS__" => $ldapsavedmsg,
        "__FORM_BEGIN_GREYCONFIG__" => $greyform->open(),
        "__FORM_CLOSE_GREYCONFIG__" => $greyform->close(),
        "__FORM_INPUTRETRYMIN__" => $greyform->input('retry_min', 10, htmlspecialchars($greylistd->getPref('retry_min'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTRETRYMAX__" => $greyform->input('retry_max', 10, htmlspecialchars($greylistd->getPref('retry_max'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTEXPIRE__" => $greyform->input('expire', 10, htmlspecialchars($greylistd->getPref('expire'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTAVOIDGREYDOMAINS__" => $greyform->input('avoid_domains', 30, htmlspecialchars($greylistd->getPref('avoid_domains'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        ...
        "__FORM_INPUTIMEOUT__" => $adform->input('smtp_receive_timeout', 3, htmlspecialchars($mta->getPref('smtp_receive_timeout'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTMAXCONNECTIONS__"  => $adform->input('smtp_accept_max', 3, htmlspecialchars($mta->getPref('smtp_accept_max'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTMAXCONNECTIONSPERHOSTS__" => $adform->input('smtp_accept_max_per_host', 3, htmlspecialchars($mta->getPref('smtp_accept_max_per_host'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTINGOREBOUNCEAFTER__" => $adform->input('ignore_bounce_after', 3, htmlspecialchars($mta->getPref('ignore_bounce_after'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTTIEMOUTFROZENAFTER__" => $adform->input('timeout_frozen_after', 3, htmlspecialchars($mta->getPref('timeout_frozen_after'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTMAXRCPT__" => $adform->input('max_rcpt', 5, htmlspecialchars($mta->getPref('max_rcpt'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')),
        "__FORM_INPUTENABLESYSLOG__" => $adform->checkbox('use_syslog', '1', htmlspecialchars($mta->getPref('use_syslog'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'), '', 1),
