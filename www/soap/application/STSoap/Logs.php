...
class STSoap_Logs {
  ...
  static public function Logs_FindLogFile($params) {
    ...
    while ($filedate < $date && $filedate != 'NODATE' && $estimate_id > -1) {
      $filename = $params['basefile'].STSoap_Logs::extFromId(--$estimate_id);
      $filedate = STSoap_Logs::getDateFromFile($filename, 'desc');
      if ($filedate == $date) {
        $filefound = $filename;
      }
    }
    ...
    while ($filedate > $date && $filedate != 'NODATE') {
      $filename = $params['basefile'].STSoap_Logs::extFromId(++$estimate_id);
      $filedate = STSoap_Logs::getDateFromFile($filename, 'asc');
      if ($filedate == $date) {
        $filefound = $filename;
      }
    }
  ...
  static public function Logs_FindLogFiles($params) {
    ...
    foreach ($params['files'] as $f) {
      $filename = STSoap_Logs::Logs_FindLogFile(array('basefile' => $f, 'date' => $params['date']));
