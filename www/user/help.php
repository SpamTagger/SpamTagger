...
// fetch help topic
$htxt = array();
$helpfile = $sysconf_->SRCDIR_."/www/user/lang/".$lang_->getLanguage()."/help_texts.php";
if (file_exists($helpfile)) {
  include($helpfile);
} else {
  $helpfile = $sysconf_->SRCDIR_."/www/user/lang/en/help_texts.php";
  if (file_exists($helpfile)) {
    include($helpfile);
  }
}
$templatehelpfile = $sysconf_->SRCDIR_."/www/user/templates/".$template_->getModel()."/lang/".$lang_->getLanguage()."/help_texts.php";
...
##http://spamtagger.org/downloads/documentations/en/body.html');
  ...
  while (preg_match('/__(MANUAL_([A-Z]+)_NAME)__/', $ret, $matches)) {
    ...
    $file = $sysconf_->SRCDIR_.'/www/user/'.$link;
    $size = 0;
    if (file_exists($file)) {
      $size = filesize($file);
    }
...
#    $ret = preg_replace("/__MANUAL_".$helpt."_LINK__/", $link, $ret);
  ...
  $ou2003abspath = $sysconf_->SRCDIR_.'/www/user/'.$ou2003relpath;
  $ou2003size = 0;
  if (file_exists($ou2003abspath)) {
  	$ou2003size = filesize($ou2003abspath);
  }
  $ou2007abspath = $sysconf_->SRCDIR_.'/www/user/'.$ou2007relpath;
  $ou2007size = 0;
