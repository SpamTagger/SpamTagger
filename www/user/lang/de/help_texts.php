...
<p>SpamTagger erfordert nur wenig Beachtung durch Sie: es merzt die Viren aus, behandelt die gef&auml;hrlichen Inhalte und entfernt die Spams aus Ihren E-Mails, jederzeit und ohne Ihr Zutun. Im Sinne der Transparenz informiert es Sie &uuml;ber seine Aktionen durch Quarant&auml;neberichte, die Sie regelm&auml;ssig in Ihrem Postfach finden.</p>

<h2>Die Quarant&auml;neberichte</h2>
<p>Ein Mal pro Tag, Woche oder Monat &mdash; je nach der Konfiguration durch Ihren Administrator oder Provider &mdash; erhalten Sie einen Bericht &uuml;ber jede Ihrer pers&ouml;nlichen Adressen, die von SpamTagger gefiltert wurde. In dieser Liste finden Sie alle E-Mails, die in der letzten Periode als Spam betrachtet und folglich in Quarant&auml;ne behalten wurden, d. h. in einer besonderen Isolationszone ausserhalb Ihres E-Mail Programms.</p>
...
<p>In sehr seltenen F&auml;llen kann es vorkommen, dass eine erw&uuml;nschte Nachricht blockiert und in Quarant&auml;ne gesetzt wurde. Dies kann verschiedene Gr&uuml;nde haben, darunter die Formatierung des E-Mails oder der schlechte Ruf des Servers, von dem aus das E-Mail versandt worden ist. Es handelt sich also nicht um eine Fehlfunktion von SpamTagger, sondern um eine Vorsichtsmassnahme angesichts der Charakteristika eines bestimmten E-Mails, die beim blossen Durchlesen des Inhalts nicht zu erkennen sein m&ouml;gen.</p>
...
<li>Mitteilung dieses Fehlers an das <i>Analysezentrum</i> von SpamTagger, damit der Filter diesem Absender gegen&uuml;ber toleranter wird. Bei SpamTagger nennen wir dies eine <i>Anpassung des Filters </i>.</li>
</ul>
...
<ul>
 ...
 <li>Sie erhalten bis zum n&auml;chsten Werktag Informationen &uuml;ber die Massnahmen, die das Analysezentrum von SpamTagger zur Korrektur des Filters getroffen hat.</li>
</ul>
...
<p>Wenn ein Spam durch die Maschen des Netzes f&auml;llt und den Weg in Ihr Postfach findet, so bedeutet dies, dass der Unterschied zu einem erw&uuml;nschten E-Mail sehr gering ist. In diesem Fall nimmt SpamTagger die Zustellung in der Annahme vor, dass es weniger schlimm ist, ein Spam zu erhalten als eine m&ouml;glicherweise wichtige Meldung nicht zu bekommen.</p>
...
<p>Sie k&ouml;nnen dem E-Mail Programm Microsoft Outlook f&uuml;r Windows eine Erweiterung (ein Plug-in) hinzuf&uuml;gen, damit von Ihrem E-Mail Programm aus ein nicht gefiltertes Spam einfach mitgeteilt werden kann. Es wird in der Men&uuml;leiste eine Schaltfl&auml;che mit dem Logo von SpamTagger und dem Text Unerw&uuml;nscht installiert.</p>

<p>Um ein Spam mit der SpamTagger Erweiterung mitzuteilen:</p>
<ul>
...
<p>Wenn Sie nicht Microsoft Outlook mit SpamTagger verwenden, so ist ein Begehren um Anpassung des Filters wegen eines nicht gefilterten Spams nur durch ein manuelles E-Mail an das Analysezentrum von SpamTagger m&ouml;glich.</p>
...
<ul>
 ...
 <li>geben Sie als Empf&auml;nger die Adresse des <i>Analysezentrums</i> von SpamTagger an, die Sie zuvor notiert haben ;</li>
...
<h2>Ihre Kenntnisse &uuml;ber SpamTagger vertiefen</h2>
<p>Wenn Sie diese paar Grundregeln gelernt haben, m&ouml;chten Sie bestimmt mehr &uuml;ber die M&ouml;glichkeiten einer benutzerdefinierten Einstellung von SpamTagger erfahren.</p>
<p>In diesem Handbuch finden Sie stets die ben&ouml;tigten Antworten.</p>
<p>Wir w&uuml;nschen Ihnen einen reibungslosen Gebrauch von SpamTagger.</p>
';

$htxt['MANUAL_FULL_NAME'] = 'spamtagger_benutzerhandbuch.pdf';
$htxt['MANUAL_FIRSTCONTACT_NAME'] = 'spamtagger_schnellstart.pdf';
$htxt['MANUAL_GENERICCONCEPT_NAME'] = 'spamtagger_allgemeine_grundsaetze.pdf';
$htxt['MANUAL_GUI_NAME'] = 'spamtagger_verwaltungsbereich.pdf';
$htxt['MANUAL_QUARANTINE_NAME'] = 'spamtagger_abschnitt_quarantaene.pdf';
$htxt['MANUAL_STATS_NAME'] = 'spamtagger_abschnitt_statistiken.pdf';
$htxt['MANUAL_CONFIGURATION_NAME'] = 'spamtagger_abschnitt_konfiguration.pdf';
$htxt['MANUAL_ERRORS_NAME'] = 'spamtagger_ungenauigkeiten_der_filterung.pdf';
...
<p>Um Strategien zur Vorbeugung und Abwehr zu verstehen, die sich im Kern des SpamTaggers befinden.</p>
...
<h3>Konfiguration</h3>
<p>Um SpamTagger Ihren Bed&uuml;rfnissen entsprechend zu personnalisieren.</p>
...
<p>F&uuml;r eine bestimmte von SpamTagger gefilterte Adresse sind Ihr Benutzername und Ihr Passwort dieselben wie in Ihrem E-Mail Programm.</p>
...
<p>Eine freiwillige Aktion von Ihnen in der Folge der Blockade einer legitimen Nachricht in der Quarant&auml;ne oder der Ankunft eines Spams in Ihrem Postfach. Im ersten Fall macht die Anpassung des Filters SpamTagger gegen&uuml;ber einem bestimmten Absender toleranter. Im zweiten Fall wird SpamTagger aggressiver.</p>
<h3>Was geschieht bei einem Begehren um Anpassung des Filters ?</h3>
<p>Eine Kopie des E-Mails wird an das Analysezentrum von SpamTagger gesandt. Die Ingenieure &uuml;berpr&uuml;fen die Lage und nehmen eventuell eine &Auml;nderung des Filters vor. Die vom Analysezentrum getroffenen Massnahmen werden Ihnen innerhalb eines Werktags mitgeteilt.</p>
...
<h3>Warum blockiert SpamTagger ein E-Mail, das mich h&auml;tte erreichen sollen ?</h3>
<p>Weil die fragliche Nachricht &uuml;ber einen Server mit zurzeit zweifelhaftem Ruf versandt wurde (er wurde vielleicht von Piraten als Relais f&uuml;r Spams verwendet) und/oder eine spezifische Formatierung enth&auml;lt, die eine Spamregel ausgel&ouml;st hat. Es handelt sich also nicht um einen Fehler von SpamTagger, sondern um eine Vorsichtsmassnahme angesichts der Charakteristika eines bestimmten E-Mails, die beim blossen Durchlesen des Inhalts nicht zu erkennen sein m&ouml;gen.</p>
...
<p>Das Analysezentrum von SpamTagger befindet sich am Sitz des Herausgebers von SpamTagger und wird jederzeit die hohe Qualit&auml;t des Filters angesichts des weltweiten Spamverkehrs, der Virenaktivit&auml;t und der Anpassungsbegehren der Benutzer aus aller Welt garantieren.</p>
...
<p>Eine freiwillige Aktion von Ihnen in der Folge der Blockade einer legitimen Nachricht in der Quarant&auml;ne oder der Ankunft eines Spams in Ihrem Postfach. Im ersten Fall macht die Anpassung des Filters SpamTagger gegen&uuml;ber einem bestimmten Absender toleranter. Im zweiten Fall wird SpamTagger aggressiver. Die Begehren werden vom Analysezentrum von SpamTagger behandelt.</p>
...
<p>Legitimes E-Mail, das von SpamTagger als Spam betrachtet wurde. Jede falsche Positivmeldung muss Gegenstand eines Begehrens um Anpassung des Filters bilden.</p>
...
<h3>Fastnet SA</h3>
<p>Der freundliche Herausgeber von SpamTagger. Das reine Gegenteil der Spammer. Sitz in St-Sulpice, Schweiz.</p>
...
<h3>Login</h3>
<p>Verfahren, das in der Feststellung besteht, ob eine Person wirklich diejenige ist, die sie zu sein vorgibt. Bei SpamTagger gestattet ein erfolgreiches Login dem Benutzer den Zugriff auf seine Quarant&auml;ne.</p>
<h3>Plug-in</h3>
<p>Englische Bezeichnung f&uuml;r eine Erweiterung, die in einer Anwendung zu installieren ist. Die Erweiterung SpamTagger f&uuml;r Microsoft Outlook vereinfacht die Mitteilungen &uuml;ber falsche Negativmeldungen.</p>
<h3>Provider</h3>
...
<p>Gesamtheit der durch dieselbe Einrichtung von SpamTagger &uuml;berwachten Internetdomains (z. B.: @meyer.com, @mueller.com).</p>
<h3>Verwaltungsbereich</h3>
...
<h6>Wenn Sie nicht Microsoft Outlook mit __LINKHELP_plugin__ SpamTagger Erweiterung__LINK__, die Kennzeichnung eines Spam fordert, dass Sie die Meldung manuell an das Analysezentrum leiten.</h6>
...
<ul>
 ...
 <li>geben Sie als Empf&auml;nger die Adresse des <em>Analysezentrums</em> von SpamTagger ein, die zuvor angegeben wurde ;</li>
