<?php
/**
 * @license https://www.gnu.org/licenses/gpl-3.0.en.html
 * @package SpamTagger
 * @author John Mertz
 * @copyright 2026, SpamTagger
 */
### newsl
$txt['NEWSLETTERMODULE'] = "Newsletter";
$txt['NEWSLETTERSTOPIC'] = "Newsletter";
$txt['NEWSLETTERSTOPICTITLE'] = "Newsletter";
$txt['NEWSLETTERSSHORTHELP'] = "Hier können Sie Ihre Newsletter verwalten.";
$txt['NEWSLETTERSALLOW'] = "Zustellen";
$txt['NEWSLETTERACCEPT'] = "Diese Newsletter annehmen";
$txt['SPAM_WANTLIST_DISABLED'] = "Die Liste ist für diese Domain nicht aktiviert (Kontaktieren Sie Ihren Administrator).";
### missing translations
$txt['FORGOTPASSWORD'] = "Passwort vergessen?";
$txt['PASSWORDRESET'] = "Das Passwort wurde zurückgesetzt. Es wurde an Ihre E-Mail-Adresse gesendet.";
/*
 * generic buttons
 */
$txt['SUBMIT'] = "Übermitteln";
$txt['CANCEL'] = "Löschen";
$txt['CLOSE'] = "Schließen";
// old
$txt['ADD'] = "Hinzufügen";
$txt['CONFIRM'] = "Bestätigen";
$txt['REFRESH'] = "Aktualisieren";
$txt['SEARCH'] = "Suchen";
$txt['SEND'] = "Senden";
// old
$txt['CLEAR'] = "Leeren";
/*
 * generic texts
 */
$txt['GB'] = "GB";
$txt['MB'] = "MB";
$txt['KB'] = "KB";
$txt['BYTES'] = "Bytes";
$txt['DATEFORMAT'] = "_D__M__Y_";
$txt['MONTHAB1'] = 'Jan.';
$txt['MONTHAB2'] = 'Feb.';
$txt['MONTHAB3'] = 'März';
$txt['MONTHAB4'] = 'Apr.';
$txt['MONTHAB5'] = 'Mai';
$txt['MONTHAB6'] = 'Juni';
$txt['MONTHAB7'] = 'Juli';
$txt['MONTHAB8'] = 'Aug.';
$txt['MONTHAB9'] = 'Sep.';
$txt['MONTHAB10'] = 'Okt.';
$txt['MONTHAB11'] = 'Nov.';
$txt['MONTHAB12'] = 'Dez.';
$txt['MONTH1'] = 'Januar';
$txt['MONTH2'] = 'Februar';
$txt['MONTH3'] = 'März';
$txt['MONTH4'] = 'April';
$txt['MONTH5'] = 'Mai';
$txt['MONTH6'] = 'Juni';
$txt['MONTH7'] = 'Juli';
$txt['MONTH8'] = 'August';
$txt['MONTH9'] = 'September';
$txt['MONTH10'] = 'Oktober';
$txt['MONTH11'] = 'November';
$txt['MONTH12'] = 'Dezember';
/*
 * login page
 */
$txt['BADLOGIN'] = "Benutzername oder Passwort falsch.";
$txt['SESSIONEXPIRED'] = "Die Sitzung ist abgelaufen.";
$txt['LOGININFO'] = "Erinnerung: Ihr Benutzername und Ihr Passwort sind identisch mit denen Ihres E-Mail-Kontos";
$txt['USERNAME'] = "Benutzername";
$txt['PASSWORD'] = "Passwort";
// old
$txt['ASKLOGIN'] = "Bitte geben Sie Ihren Benutzernamen und Ihr Passwort ein:";
$txt['ORGANIZATIONUNIT'] = "Organisationseinheit";
/*
 * menu
 */
$txt['CONFIGURATIONMENU'] = "Konfiguration";
$txt['HELPMENU'] = "Hilfe";
$txt['QUARANTINEMENU'] = "Quarantäne";
$txt['STATISTICSMENU'] = "Statistiken";
$txt['LOGOUT'] = "Abmelden";
//old
$txt['LOGGEDAS'] = "Sie sind angemeldet als:";
// old
$txt['MAINADDRESS'] = "Hauptadresse";
$txt['PARAMETERS'] = "Einstellungen";
$txt['QUARANTINE'] = "Quarantäne";
$txt['NAVSUPPORT'] = "Support";
/*
 * quarantaine
 */
$txt['SELECTEDADDRESS'] = "Angezeigte Adresse";
$txt['QUARANTINESUMMARY'] = "<strong>In Quarantäne:</strong> __PARAM__ Meldungen";
$txt['SEARCHSUMMARY'] = "<strong>Suche:</strong> __PARAM__ Meldungen";
$txt['ORDEREDBYPARAM'] = "Sortiert nach __PARAM__";
$txt['RESETSEARCH'] = "Gesamte Quarant&auml;ne Liste nochmals anzeigen";
$txt['ODATE'] = "Datum";
$txt['OTIME'] = "Zeit";
$txt['OSCORE'] = "Score";
$txt['ODESTINATION'] = "Empfänger";
$txt['OSENDER'] = "Absender";
$txt['OSCORE'] = "Bewertung";
$txt['OSUBJECT'] = "Betreff";
//old
$txt['RESETSEARCH'] = "Gesamte Quarantäneliste erneut anzeigen";
//old
$txt['CRITERIAS'] = "Filterregeln";
$txt['FORCEMESSAGE'] = "Meldung freigeben";
$txt['MESSAGEFORCING'] = "Meldung freigeben";
$txt['MESSAGEPREVIEW'] = "Vorschau der Meldung";
$txt['ANALYSEREQUEST'] = "Filteranpassung anfordern";
$txt['ANALYSEREQUESTV'] = "Filteranpassung anfordern";
$txt['SCORETEXT'] = "Bewertung: __PARAM__";
$txt['CLOSEWINDOW'] = "Fenster schließen";
$txt['CURRENTPAGE'] = "Seite __PARAM1__ von __PARAM2__";
$txt['PURGEINFOS'] = "<strong>Automatisch Säubern nach:</strong> __PARAM__ Tage";
$txt['DISPLAYEDINFOS'] = "<strong>angezeigte Meldungen: </strong>letzte __PARAM1__ Tage (<a href=\"__PARAM2__\">&Auml;ndern...</a>). ";
$txt['DISPLAYEDINFOSS'] = "<strong>Angezeigte Meldungen: </strong>letzte __PARAM__ Tage. ";
// old
$txt['QUARANTINETITLE'] = "Quarantäneliste für __PARAM__";
// old
$txt['FILTER'] = "Filter";
// old
$txt['ADVANCEDSEARCH'] = "Erweiterte Suche";
// old
$txt['SHOWEDMESSAGES'] = "Angezeigte Meldungen";
// old
$txt['COMINGFROM'] = "Von";
$txt['DATE'] = "Datum";
$txt['HOUR'] = "Zeit";
$txt['FROM'] = "Von";
$txt['TO'] = "An";
$txt['SUBJECT'] = "Betreff";
$txt['SENDER'] = "Absender";
$txt['FORCED'] = "Freigegeben";
$txt['ACTION'] = "Aktion";
// old
$txt['TOTALSPAMS'] = "Gesamt: __PARAM__ Spam";
// old
$txt['FORTHEXLASTDAYS'] = "Für die letzten __PARAM__ Tage";
$txt['PURGESELECTEDSPAMS'] = "Manuelle Bereinigung der Quarantäne";
// old
$txt['HIDEFORCED'] = "Bereits freigegebene Meldungen ausblenden";
// old
$txt['FORCESPAM'] = "Meldung freigeben";
// old
$txt['ASKREASON'] = "Filterregeln anzeigen";
$txt['CONFSENDANALYSE'] = "Ein Antrag auf Filteranpassung wird an das SpamTagger Analysis Center gesendet; er enthält eine Kopie der Nachricht, die fälschlicherweise blockiert wurde.</br>Hinweis: Eine Filteranpassung ist nicht dasselbe wie eine Nachricht zu Weißen oder Schwarzen Liste verschieben.";
$txt['AREYOUSURE'] = "<span class=\"Frage\">W&uuml;nschen Sie wirklich die Analyse der Meldung und dass entsprechende Filterungsanpassungen angewendet werden ?</span>";
$txt['ASKANALYSE'] = "Filteranpassung anfordern";
$txt['PAGE'] = "Seite";
$txt['NEXTPAGE'] = "Weiter";
$txt['PREVIOUSPAGE'] = "Zurück";
// old
$txt['GETREASONSTITLE'] = "Gründe für die Filterung";
$txt['HITRULE'] = "Regel";
$txt['SCORE'] = "Bewertung";
$txt['TOTAL'] = "Gesamt";
//old
$txt['SENDANALYSETITLE'] = "Filteranpassung anfordern";
$txt['SENTTOANALYSE'] = "Das Begehren um Anpassung des Filters wurde an das Analysezentrum gesandt. Diese Meldung wird innerhalb eines Arbeitstages gepr&uuml;ft, und nach den Ergebnissen, werden eventuelle Filterungsanpassungen angebracht.";
/*
 * quick actions
 */
//old
$txt['SENDSUMTITLE'] = "Quarantänebericht manuell versenden";
$txt['SENDSUM'] = "Quarantänebericht manuell versenden";
$txt['SUMSENTTO'] = "Der Quarantänebericht wurde an die Adresse <strong>__PARAM__</strong> gesandt.";
$txt['SUMNOTSENTTO'] = "Der Quarant&auml;nebericht wurde an die Adresse: <strong>__PARAM__</strong> aus Fehlergr&uuml;nde nicht gesandt. Bitte versuchen Sie es noch einmal.";
$txt['SUMMARYSENDING'] = "Quarantäneberichte übermitteln";
$txt['PURGETITLE'] = "Quarantäne manuell bereinigen";
$txt['PURGEDONE'] = "Die Quarant&auml;ne wurde f&uuml;r die Adresse: <strong>__PARAM__</strong> gel&ouml;scht";
$txt['COULDNOTPURGE'] = "Die Quarant&auml;ne verbunden mit der Adresse <strong>__PARAM__</strong> wurde, aus Fehlergr&uuml;nden nicht gel&ouml;scht. Bitte versuchen Sie es noch einmal.";
$txt['ASKPURGECONFIRM'] = "M&ouml;chten Sie wirklich den Inhalt der Quarant&auml;ne f&uuml;r die letzten __PARAM1__ Tage f&uuml;r die Adresse <strong>__PARAM2__</strong> l&ouml;schen ?";
$txt['QUARANTINEPURGE'] = "Quarantäne manuell bereinigen";
$txt['GROUPQUARANTINES'] = "Alle Adressen";
/*
 * preview panel
 */
$txt['INFORMATIONSABOUTMSG'] = "Inhalt der Nachricht";
$txt['PREFILTERHITS'] = "Vor-Filterung";
$txt['BLOCKLISTS'] = "Schwarze Listen";
$txt['FITLERSCORE'] = "Bewertung";
$txt['NONE'] = "Kein";
$txt['HEADERS'] = "Kopfzeile";
$txt['BODY'] = "Inhalt";
$txt['PARTS'] = "Teile";
$txt['STOREREPLICA'] = "Speicherung";
$txt['SPAMCSCORE'] = "Regelwertung";
$txt['DESCRIPTION'] = "Beschreibung";
$txt['MESSAGEPREVIEW'] = "Nachrichtenvorschau";
$txt['ID'] = "Erkennung";
/*
 * logout page
 */
$txt['LOGOUTTEXT'] = "<p>Ihre SpamTagger Session ist beendet.</p><p>Wenn Sie erneut anmelden m&ouml;chten, bitte tippen Sie Folgendes:</p>";
/*
 * parameters page
 */
// old
$txt['PARAMTITLE'] = "Pers&ouml;nliche Einstellungen von SpamTagger";
// old
$txt['USERPARAM'] = "Benutzereinstellungen";
// old
$txt['ADDRESSPARAM'] = "Adresseinstellungen";
// old
$txt['LANGUAGE'] = "Sprache";
$txt['ADDRESS'] = "Adresse";
// old
$txt['ADDRALIASTITLE'] = "Adressgruppe";
// old
$txt['FILTERACTIONTITLE'] = "Spam-Verarbeitungsmodus";
// old
$txt['SUMMARYPARAMSTITLE'] = "Quarantäneberichteinstellungen";
// old
$txt['ADDRESSESLISTTITLE'] = "Adressen oder pers&ouml;nliche Alias: (__PARAM__ Addresse(n))";
// old
$txt['APPLYALLADDRESSES'] = "Änderungen auf alle Adressen anwenden";
// old
$txt['MAIN'] = "Hauptadresse";
// old
$txt['SPAMACTION'] = "Spamaktion";
// old
$txt['PUTINQUARANTINE'] = "Unter Quarant&auml;ne stellen";
// old
$txt['TAGSUBJECT'] = "Meldung markieren";
// old
$txt['QUARBOUNCES'] = "Fehlermeldungen in Quarant&auml;ne gestellt";
// old
$txt['DROP'] = "L&ouml;schen";
// old
$txt['SUBJECTTAG'] = "Pers&ouml;nliche Bezeichnung der Betreffzeile";
// old
$txt['SUMMARYFREQ'] = "H&auml;ufigkeit der Berichte";
$txt['DAILY'] = "Täglich";
$txt['WEEKLY'] = "Wöchentlich";
$txt['MONTHLY'] = "Monatlich";
$txt['NOSUMMARY'] = "Kein Bericht";
// old
$txt['SUMMARYTYPE'] = "Typus des Berichts";
$txt['USEDEFAULT'] = "Standardm&auml;ssig";
$txt['SUMMHTML'] = "HTML";
$txt['SUMMTEXT'] = "Reintext";
// old
$txt['EDITWANTLIST'] = "Positivliste bearbeiten";
// old
$txt['EDITWARNLIST'] = "Warnliste verwalten";
/*
 * wwlist management
 */
// old
$txt['SENDER'] = "Absender";
// old
$txt['COMMENT'] = "Erl&auml;uterung";
// old
$txt['ACTIONS'] = "Aktionen";
// old
$txt['ACTIVE'] = "aktiviert";
// old
$txt['INNACTIVE'] = "desaktiviert";
// old
$txt['WWENTRY'] = "Eingabe";
// old
$txt['CONFIRMWWENTRYDELETE'] = "Sind Sie sicher, die Adresse endg&uuml;ltig l&ouml;schen zu wollen ?";
// old
$txt['WANTLISTFOR'] = "Wei&szlig;e Liste f&uuml;r __PARAM__";
// old
$txt['WARNLISTFOR'] = "Warnliste f&uuml;r __PARAM__";
// old
$txt['STATUS'] = "Status";
/*
 * alias stuff
 */
// old
$txt['ADDALIASTITLE'] = "Adresse/Alias beif&uuml;gen";
// old
$txt['ADDALIASFORM'] = "Adresse/Alias";
// old
$txt['REMADDRESSALT'] = "Adresse l&ouml;schen";
// old
$txt['ADDADDRESSALT'] = "Adresse beif&uuml;gen";
$txt['BADADDRESSFORMAT'] = "Eingegebene Adresse ung&uuml;ltig. Bitte geben Sie die Adresse nochmal ein.";
$txt['NOTFILTEREDDOMAIN'] = "Dieses Domain ist vom SpamTagger nicht filtriert. Demzufolge kann die Adresse nicht zugef&uuml;gt werden.";
$txt['ALIASALREADYREGISTERD'] = "Adresse bereits registriert! Diese Adresse wird nicht zugef&uuml;gt, denn sie ist bereits von SpamTagger gepr&uuml;ft.";
$txt['ALIASALREADYPENDING'] = "Eine Anfrage f&uuml;r diese Adresse ist bereits vorhanden.";
$txt['ALIASPENDING'] = "An diese Adresse wurde eine Bestätigung gesendet.<br/> Aus Sicherheitsgründen wird die Adresse nur dann hinzugefügt, wenn Sie die Anfrage innerhalb von 24 Stunden bestätigen.";
$txt['ALIASREQUESTSUBJECT'] = "[SpamTagger] Anfrage um Adresse/Alias beizuf&uuml;gen";
$txt['ALIASERRORSENDIG'] = "Die Anfrage konnte aufgrund eines internen Fehlers nicht gesendet werden. Bitte versuchen Sie es später noch einmal.";
$txt['ALIASNOTPENDING'] = "F&uuml;r das Zuf&uuml;gen dieser Adresse wurde keine Anfrage gemacht.";
$txt['ALIASADDED'] = "Adresse wurde erfolgreich zugef&uuml;gt.";
$txt['ALIASREQUESTREMOVED'] = "Anfrage um Adresse Beizuf&uuml;gen ist gel&ouml;scht.";
// old
$txt['REMALIASTITLE'] = "R&uuml;cknahme der Adresse/Alias";
// old
$txt['REMALIASCONFIRM'] = "R&uuml;cknahme der Adresse __PARAM__ best&auml;tigen ?";
$txt['ALIASREMOVED'] = "Adresse __PARAM__ zur&uuml;ckgezogen.";
$txt['CANNOTREMOVEMAINADD'] = "Die Adresse __PARAM__ ist die Hauptadresse dieses SpamTagger Kontos, die nicht zur&uuml;ckgezogen werden kann.";
/*
 * statistics page
 */
// old
$txt['USERSTATS'] = "Statistiken";
// old
$txt['USERMESGSSTAT'] = "__PARAM__ empfangene Meldungen";
// old
$txt['USERSPAMSSTAT'] = "__PARAM__ Spams";
// old
$txt['USERSDANGEROUSSTAT'] = "__PARAM__ gef&auml;hrliche";
// old
$txt['USERCLEANSTAT'] = "__PARAM__ saubere";
$txt['ALL'] = "Alle…";
$txt['SEARCHPERIOD'] = "Gepr&uuml;fte Zeitspanne";
$txt['LASTDAYS'] = "letzte Tage";
$txt['LASTMONTHS'] = "letzte Monate";
$txt['LASTYEARS'] = "letzte Jahre";
$txt['DATESTART'] = "Von";
$txt['DATESTOP'] = "bis";
$txt['STATFORADDRESS'] = "T&auml;tigkeitsstatistiken f&uuml;r die Adresse: <strong>__PARAM__</strong>";
$txt['RECEIVEDMESSAGES'] = "Empfangene Meldungen";
$txt['RECEIVEDVIRUS'] = "Meldungen mit Viren und gef&auml;hrlichem Inhalt";
$txt['RECEIVEDSPAM'] = "Meldungen, die als Spams betrachtet wurden";
$txt['RECEIVEDCLEAN'] = "Saubere Meldungen";
$txt['NBPROCESSEDMSGS'] = "__PARAM__ empfangene Meldungen";
$txt['GLOBALSTATSTITLE'] = "Zusammenfassung f&uuml;r alle Adresse";
$txt['FROMDATETODATE'] = "Von __PARAM1__.__PARAM2__.__PARAM3__ bis __PARAM4__.__PARAM5__.__PARAM6__";
/*
 * some errors messages
 */
$txt['BADARGS'] = "Interner Fehler des SpamTaggers (__PARAM__). Wenden Sie sich an Ihrer E-Mail-Administrator.";
$txt['INCORRECTMSGID'] = "Die gew&uuml;nschte Aktion kann nicht ausgef&uuml;hrt werden, weil die Meldungserkennung falsch ist.";
$txt['NOSUCHADDR'] = "Die angegebene Adresse ist von SpamTagger nicht gepr&uuml;ft.";
$txt['ERRORSENDING'] = "Fehler beim Absenden der Nachricht aufgetreten. Bitte versuchen Sie es noch einmal. Wenn der Fehler wiederum eintritt, wenden Sie sich an Ihren E-Mail-Administrator.";
$txt['MSGFORCED'] = "Meldung wurde freigegeben.<br/>Sie wird in wenigen Augenblicken in Ihren Postfach eingeworfen.";
$txt['MSGFILENOTFOUND'] = "Die gew&uuml;nschte Aktion kann nicht ausgef&uuml;hrt werden, weil die vorbemerkte Meldung in der Quarant&auml;ne nicht mehr vorhanden ist.";
/*
 * support page (deprecated)
 */
// old
$txt['COMPANY'] = "Firma";
// old
$txt['NAME'] = "Name";
// old
$txt['FIRSTNAME'] = "Vorname";
// old
$txt['EMAIL'] = "E-Mail";
// old
$txt['YOURPHONENUMBER'] = "Ihre Telefonnummer";
// old
$txt['WHATCANWEDO'] = "Was kann man f&uuml;r Sie tun ?";
// old
$txt['NEEDEDFIELDS'] = "Antwort ben&ouml;ntigt";
// old
$txt['SUPPORT'] = "Support";
// old
$txt['SUPFORMSENT'] = "Ihre Meldung wurde gesendet.";
//old
$txt['CANNOTSENDSUPFORM'] = "Ihre Meldung wurde nicht geschickt Votre message n'a pas pu &ecirc;tre envoy&eacute;. Wenden Sie sich an Ihren systemadministrator.";
//old
$txt['BADFORMFIELDS'] = "Einige Felder sind fehlerhaft. Bitte überprüfen Sie Ihre Eingaben.";
/*
 * help page (new)
 */
$txt['HELP'] = "Hilfe";
$txt['INTERFACETOPIC'] = "Schnittstelle";
$txt['INTRODUCTIONTOPIC'] = "Einleitung";
$txt['INTRODUCTIONTOPICTITLE'] = "Einleitung";
$txt['FIRSTCONTACTTOPIC'] = "Schnellstart";
$txt['FIRSTCONTACTTOPICTITLE'] = "Schnellstart";
$txt['ADDRESSESTOPIC'] = "N&uuml;tzliche Adresse";
$txt['ADDRESSESTOPICTITLE'] = "n&uuml;tzliche Adresse";
$txt['PLUGINTOPIC'] = "Outlook Extension";
$txt['PLUGINTOPICTITLE'] = "Outlook Extension";
$txt['MOREHELPTOPIC'] = "Gesamtes Handbuch";
$txt['MOREHELPTOPICTITLE'] = "gesamtes Handbuch";
$txt['USERMANUALTOPICTITLE'] = "Benutzerhandbuch";
$txt['USERMANUALTOPIC'] = "Benutzerhandbuch";
$txt['FAQTOPIC'] = "H&auml;ufig gestellte Fragen";
$txt['FAQTOPICTITLE'] = "h&auml;ufig gestellte Fragen";
$txt['GLOSSARYTOPIC'] = "Glossar";
$txt['GLOSSARYTOPICTITLE'] = "Glossar";
$txt['SUPPORTTOPIC'] = "Support";
$txt['SUPPORTTOPICTITLE'] = "Support";
$txt['ANALYSETOPIC'] = "Begehren um Anpassung";
$txt['ANALYSETOPICTITLE'] = "Begehren um Anpassung";
/*
 * configuration menu (new)
 */
$txt['CONFIGURATION'] = "Konfiguration";
$txt['ADDRESSLISTTOPIC'] = "Adressgruppe";
$txt['ADDRESSPARAMTOPIC'] = "Filterungsmodus";
$txt['QUARPARAMTOPIC'] = "Anzeige der Quarant&auml;ne";
$txt['WARNLISTTOPIC'] = "Warnliste";
$txt['WANTLISTTOPIC'] = "Wei&szlig;e Liste";
$txt['BLOCKLISTTOPIC'] = "Schwarze Liste";
$txt['INTERFACETOPICTITLE'] = "Schnittstelle";
$txt['ADDRESSLISTTOPICTITLE'] = "Adressgruppe";
$txt['ADDRESSPARAMTOPICTITLE'] = "Filterungsmodus";
$txt['QUARPARAMTOPICTITLE'] = "Anzeige der Quarant&auml;ne";
$txt['WARNLISTTOPICTITLE'] = "Warnliste";
$txt['WANTLISTTOPICTITLE'] = "Wei&szlig;e Liste";
$txt['BLOCKLISTTOPICTITLE'] = "Schwarze Liste";
/*
 * interface (new)
 */
$txt['SAVE'] = "Speichern";
$txt['CHOOSETHISLANG'] = "W&auml;hlen Sie die deutsche Sprache als Navigationssprache";
$txt['CHOOSELANGUAGE'] = "Sprache der Schnittstelle";
/*
 * aliases (new)
 */
$txt['ADDLISTSHORTHELP'] = "Diese Gruppe enth&auml;lt Adressen und Aliases, die Sie von diesem Konto verwalten m&ouml;chten. Sie k&ouml;nnen auf alle Ihre Adressen unter Verwendung Ihres Benutzernamens und Passwortes zentral zugreifen.";
$txt['ADDANADDRESS'] = "Hinzuf&uuml;gen einer Adresse zur Gruppe";
$txt['ADDTHEADDRESS'] = "&lt; Adresse zur Gruppe hinzuf&uuml;gen";
$txt['ADDANADDRESSSHORTHELP'] = "Adresse eintragen, die Sie Ihrer Gruppe zuf&uuml;gen m&ouml;chten.";
$txt['REMANADDRESS'] = "Einer oder mehrerer Adressen aus der Gruppe entfernen";
$txt['REMTHEADDRESS'] = "Auswahl aufheben";
$txt['REMANADDRESSSHORTHELP'] = "Aktivieren Sie das Kontrollk&auml;stchen jeder zu l&ouml;schenden Adresse und dann klicken Sie auf die entsprechende Schaltfl&auml;che.";
$txt['WAITINGCONFIRMATION'] = "Warten auf Bestätigung";
/*
 * addresses settings (new)
 */
$txt['ADDPARAMSHORTHELP'] = "Der Filterungsmodus von SpamTagger ist f&uuml;r jede Ihrer Adressen oder Aliases m&ouml;glich. F&uuml;r eine genaue Beschreibung der verschiedenen Filterungsmodus, wenden Sie sich an das Benutzerhandbuch(verf&uuml;gbar im Abschnitt <a href=\"help.php\">\"Hilfe\"</a>).";
$txt['ADDPARAMSHORTHELP'] = "Der Filterungsmodus von SpamTagger ist f&uuml;r jede Ihrer Adressen oder Aliases m&ouml;glich. F&uuml;r eine genaue Beschreibung der verschiedenen Filterungsmodus, wenden Sie sich an das Benutzerhandbuch(verf&uuml;gbar im Abschnitt <a href=\"help.php\">„Hilfe“</a>).";
$txt['FOREACHSPAMDO'] = "Für jede als Spam erkannte Nachricht:";
$txt['FOREACHNEWSLETTERDO'] = "Für jede Nachricht, die als Newsletter erkannt wurde:";
$txt['KEEPMESSAGEINQUARANTINE'] = "Zur&uuml;ckhalten der Meldung in Quarant&auml;ne";
$txt['TAGMESSAGEWITHTAG'] = "Meldung einwerfen, in der das Betreff mit dem Pr&auml;fix markiert ist";
$txt['DROPMESSAGE'] = "Meldung sofort l&ouml;schen";
$txt['SUMMARYSENTFREQ'] = "Versand der Quarant&auml;neberichten an die ausgew&auml;hlte Adresse";
$txt['SUMMARYSENTFREQ'] = "Versand der Quarant&auml;neberichten an die ausgew&auml;hlte Adresse";
$txt['SUMMARYFORMAT'] = "Versand der Quarant&auml;neberichten mit dem Format";
$txt['SUMMARYTO'] = "Berichte an diese Adresse senden";
$txt['OTHER'] = "Weitere …";
$txt['APPLYTOALLADDRESSES'] = "Anwenden dieser Regeln f&uuml;r alle E-Mail-Adressen";
$txt['PLAINTEXT'] = "Brutto-Text";
$txt['HTML'] = "Formatiert (HTML)";
$txt['DIGEST'] = "Digest";
$txt['KEEPBOUNCESINQUARANTINE'] = "Systematisches Zur&uuml;ckhalten der Fehlermeldungen aus Empfangs-Server zur&uuml;ckgeschickt";
$txt['PARAMETERSSAVED'] = "Parameter wurden gespeichert und sofort angewandt.";
$txt['NOTSAVED'] = "Ihre Einstellungen konnten nicht gespeichert werden.";
$txt['INVALIDSUMMARYTO'] = "Die für die Berichte angegebene E-Mail-Adresse ist ungültig.";
/*
 * quarantine  (new)
 */
$txt['CONFIGQUARSHORTHELP'] = "Anzeige der Quarant&auml;neparameter (Anzahl Tage und Zeilen) kann Ihren Bed&uuml;rfnisse und Gewohnheiten entsprechend angepasst werden.";
$txt['QUARNBLINESDISPLAYED'] = "Anzahl der angezeigten Zeilen";
$txt['QUARNBDAYSDISAPLYED'] = "Anzahl der angezeigten Tage";
$txt['MASKEALREADYFORCED'] = "Meldungen ausblenden, die bereits vom Benutzer freigegeben wurden";
$txt['DEFAULTADDRESDISPPLAYED'] = "Adresse standardm&auml;ssig angezeigt";
$txt['YESTERDAY'] = "Gestern";
$txt['TODAY'] = "Heute";
/*
 * wwlists (new)
 */
$txt['WARNLISTSHORTHELP'] = "Die Warnliste enthält vertrauenswürdige Absenderadressen, für die Sie per E-Mail benachrichtigt werden möchten, wenn eine Nachricht als Spam erkannt wird. Sie können die Nachricht dann im Falle eines Filterfehlers sofort freigeben.<br />
\t\t\t\t\t\t\t\t   Warnhinweis: Diese Funktionalität sollte als temporäre Lösung verwendet werden, da sie den Gesamtschutz verringert.
\t\t\t\t\t\t\t\t   Weitere Informationen zur Verwendung der Warnliste finden Sie im Handbuch (siehe Abschnitt <a href=\"help.php\">„Hilfe“</a>). ";
$txt['WARNLISTFORADDRESS'] = "Warnliste f&uuml;r die Adresse";
$txt['ADDAWARNENTRY'] = "Adresse in die Warnliste zuf&uuml;gen";
$txt['WARNLISTENTRYTOADD'] = "Adresse eintragen, die Sie zur Warnliste zuf&uuml;gen m&ouml;chten. Sie k&ouml;nnen einen zus&auml;tzlichen Kommentar anf&uuml;gen.";
$txt['ADDTHEENTRY'] = "&lt; Adresse zuf&uuml;gen";
$txt['REMOVEANENTRY'] = "Adresse l&ouml;schen, desaktivieren oder wieder-aktivieren";
$txt['REMOVEAWARNENTRYSHORTHELP'] = "W&auml;hlen Sie in der Warnliste die Adresse(n) die Sie &auml;ndern m&ouml;chten, dann klicken Sie auf die entsprechende Schaltfl&auml;che.";
$txt['REMTHEENTRY'] = "Auswahl l&ouml;schen";
$txt['DISABLETHEENTRY'] = "Auswahl desaktivieren oder wieder-aktivieren";
$txt['WANTLISTSHORTHELP'] = "Die Positivliste enthält vertrauenswürdige Absenderadressen, für die keine Quarantäneeinstellungen gelten sollen.<br />
                                   Achtung: Diese Funktionalität sollte als vorübergehende Lösung verwendet werden, da sie den Gesamtschutz verringert.
                                  Weitere Informationen zur Verwendung der Positivliste finden Sie im Handbuch (siehe Abschnitt <a href=\"help.php\">\"Help\"</a>).
";
$txt['BLOCKLISTSHORTHELP'] = "Die schwarze Liste enthält die gesperrte Absenderadressen …
                                  Lesen Sie das Handbuch (verfügbar im Abschnitt <a href=\"help.php\">\"Hilfe\"</a>) für weitere Informationen zur Verwendung der schwarzen Liste.
";
$txt['WANTLISTFORADDRESS'] = "Wei&szlig;e Liste f&uuml;r die Adresse";
$txt['ADDAWANTENTRY'] = "Adresse in die Wei&szlig;e Liste zuf&uuml;gen";
$txt['WANTLISTENTRYTOADD'] = "Adresse eintragen, die Sie zur Wei&szlig;e Liste zuf&uuml;gen m&ouml;chten. Sie k&ouml;nnen einen zus&auml;tzlichen Kommentar anf&uuml;gen.";
$txt['REMOVEAWANTENTRYSHORTHELP'] = "W&auml;hlen Sie in der Wei&szlig;e Liste die Adresse(n) die Sie &auml;ndern m&ouml;chten, dann klicken Sie auf die entsprechende Schaltfl&auml;che.";
$txt['BLOCKLISTFORADDRESS'] = "Schwarze Liste für die Adresse";
$txt['ADDABLOCKENTRY'] = "Fügen Sie eine Adresse in die Liste";
$txt['BLOCKLISTENTRYTOADD'] = "Geben Sie die Adresse, die Sie auf die schwarze Liste hinzufügen möchten. Sie können auch einen optionalen Kommentar ein.";
$txt['REMOVEABLOCKENTRYSHORTHELP'] = "Wählen Sie die Adresse oder Adressen, die Sie aus der Liste ändern möchten, und klicken Sie auf die gewünschte Aktion Taste.";
$txt['RECORDALREADYEXISTS'] = "Diese Adresse ist bereits in der Liste vorhanden.";
/*
 * logout (new)
 */
$txt['SESSIONTERMINATED'] = "Ihre SpamTagger Session ist beendet.";
$txt['BEENLOGGEDOUT'] = "Sie k&ouml;nnen sich wieder einloggen an die folgende Adresse: <a href=\"__PARAM__\" >__PARAM__</a>";
/*
 * Newsletters release
 */
$txt['NLRELEASEDHEAD'] = "Newsletter wurde abonniert und gesendet.";
$txt['NLRELEASEDBODY'] = "Newsletter wurde abonniert und gesendet. Es sollte in wenigen Minuten in Ihrem E-Mail-Postfach liegen.";
$txt['NLNOTRELEASEDHEAD'] = "Newsletter wurde nicht versendet";
$txt['NLNOTRELEASEDBODY'] = "Es gab ein Problem bei der Freigabe Ihrer Nachricht.";
//old
$txt['EDITNEWSLIST'] = "Newslist editieren";
//old
$txt['NEWSLISTFOR'] = "Newsletterliste f&uuml;r __PARAM__";
$txt['NEWSLISTTOPIC'] = "Liste der Newsletter";
$txt['NEWSLISTTOPICTITLE'] = "Liste der Newsletter";
$txt['NEWSLISTSHORTHELP'] = "Die Newsletter-Liste enthält Adressen der gewünschten Newsletter.<br />
                                  Weitere Informationen zur Verwendung der Newsletterliste finden Sie im Handbuch (verfügbar im Abschnitt <a href=\"help.php\">\"Help\"</a>).
";
$txt['NEWSLISTFORADDRESS'] = "Newsletter-Liste für die Adresse";
$txt['ADDANEWSENTRY'] = "Eine Adresse zur Liste hinzufügen";
$txt['NEWSLISTENTRYTOADD'] = "Geben Sie die Adresse ein, die Sie in die Newsletterliste aufnehmen möchten. Sie können auch einen optionalen Kommentar hinzufügen.";
$txt['REMOVEANEWSENTRYSHORTHELP'] = "Wählen Sie die Adresse oder die Adressen, die Sie ändern möchten, aus der Liste aus und klicken Sie auf die gewünschte Aktionsschaltfläche.";
$txt['ADDTOGROUP'] = "Für alle Adressen in der Gruppe hinzufügen";
$txt['NEWSLONLY'] = "Nur Newsletter anzeigen";
$txt['SPAMONLY'] = "Nur Spam anzeigen";
/*
 * Newsletter rule
 */
$txt['ADDNEWSLIST'] = "Alle zukünftigen Newsletter vom gleichen Absender akzeptieren?";
$txt['UNKNOWNERROR'] = "Ein unbekannter Fehler ist aufgetreten.";
$txt['SENDERNOTVALID'] = "Ungültige Absenderadresse.";
$txt['DESTNOTVALID'] = "Ungültige Zieladresse.";
$txt['DUPLICATEENTRY'] = "Ein doppelter Eintrag existiert schon.";
$txt['CONFIGREADFAIL'] = "Konfigurationsdatei konnte nicht gelesen werden.";
$txt['CANNOTINSERTDB'] = "Fehler beim Einfügen in die Datenbank.";
$txt['CANNOTSELECTDB'] = "Fehler beim Durchsuchen der Datenbank.";
$txt['CANNOTCONNECTDB'] = "Fehler beim Herstellen einer Verbindung mit der Datenbank.";
/*
 * SOAP errors
 */
$txt['CANNOTLOADMESSAGE'] = "Fehler beim Laden der Nachricht (sie existiert möglicherweise nicht mehr).";
$txt['NEWSWANTLISTBODY'] = "Eine Newslist- und Wantlist-Regel wurden hinzugefügt. Zukünftige Nachrichten dieses Absenders werden nicht markiert, wenn sie als Newsletter oder Spam erkannt werden.";
$txt['NEWSWANTLISTHEAD'] = "Newslist- und Wantlist-Regeln wurden hinzugefügt.";
$txt['ADDWANTNEWSLIST'] = "Alle zukünftigen Spams desselben Absenders auf die Wantlist setzen? Auch Newsletter von diesem Absender akzeptieren?";
/*
 * Newslist and Wantlist rule
 */
$txt['ADDNEWSWANTLIST'] = "Alle zukünftigen Newsletter vom gleichen Absender akzeptieren? Auch alle Spam-Nachrichten dieses Absenders auf die Wantlist setzen?";
$txt['NOTWANTLISTBODY'] = "Beim Hinzufügen der Wantlist ist ein Problem aufgetreten.";
$txt['NOTWANTLISTHEAD'] = "Wantlist-Regel wurde nicht hinzugefügt.";
$txt['WANTLISTBODY'] = "Eine Wantlist-Regel wurde hinzugefügt. Zukünftige Nachrichten von diesem Absender werden nicht markiert, wenn sie als Spam erkannt werden.";
$txt['NEWSNOTWANTBODY'] = "Eine Newsletter-Regel wurde hinzugefügt, beim Hinzufügen der Wantlist-Regel ist jedoch ein Problem aufgetreten.";
$txt['NEWSNOTWANTHEAD'] = "Newslist wurde hinzugefügt, Wantlist jedoch nicht.";
$txt['ADDRULE'] = "WWList-Regel hinzufügen";
//
$txt['ADDITIONALACTION'] = "Zusätzliche Aktionen:";
$txt['NOTBLOCKLISTBODY'] = "Beim Hinzufügen der schwarzen Liste ist ein Problem aufgetreten.";
$txt['NOTBLOCKLISTHEAD'] = "Blocklist-Regel wurde nicht hinzugefügt.";
$txt['BLOCKLISTBODY'] = "Eine Blocklist-Regel wurde hinzugefügt. Zukünftige Nachrichten von diesem Absender werden immer als Spam gekennzeichnet.";
$txt['BLOCKLISTHEAD'] = "Blocklist-Regel wurde hinzugefügt.";
/*
 * Blocklist rule (not currently implemented)
 */
$txt['ADDBLOCKLIST'] = "Alle zukünftigen E-Mails desselben Absenders auf die schwarze Liste setzen?";
$txt['NOTNEWSWANTBODY'] = "Beim Hinzufügen dieser Regeln ist ein Problem aufgetreten.";
$txt['NOTNEWSWANTHEAD'] = "Weder Newslist- noch Wantlist-Regeln wurden nicht hinzugefügt.";
$txt['WANTNOTNEWSBODY'] = "Eine Wantlist-Regel wurde hinzugefügt, beim Hinzufügen der Newsletter-Wantlist-Regel ist jedoch ein Problem aufgetreten.";
$txt['WANTNOTNEWSHEAD'] = "Wantlist wurde hinzugefügt, Newslist jedoch nicht.";
/*
 * Type of WWList
 */
$txt['ORIGINALSENDER'] = "Nur ursprüngliche Absenderadresse";
$txt['SENDERVARIATIONS'] = "Alle Variationen der Absenderadresse";
$txt['ENTIREDOMAIN'] = "Gesamte Domain";
$txt['NEWSLISTHEAD'] = "Nachrichtenlisten-Regel wurde hinzugefügt.";
/*
 * generic texts
 */
$txt['AND'] = "und";
$txt['NOTNEWSLISTHEAD'] = "Nachrichtenlisten-Regel wurde nicht hinzugefügt.";
$txt['NEWSLETTERUNSUB'] = "Abbestellen";
$txt['NOTNEWSLISTBODY'] = "Es gab ein Problem beim Hinzufügen der Newsletter-Wantlist.";
$txt['WANTLISTHEAD'] = "Es wurde eine Wantlist-Regel hinzugefügt.";
$txt['ADDWANTLIST'] = "Alle zukünftigen Spamnachrichten vom selben Absender auf die Wantlist setzen?";
$txt['NEWSLISTBODY'] = "Es wurde eine Wantlist-Regel für Newsletter hinzugefügt. Zukünftige Nachrichten von diesem Absender werden nicht gekennzeichnet, wenn sie als Newsletter erkannt werden.";
