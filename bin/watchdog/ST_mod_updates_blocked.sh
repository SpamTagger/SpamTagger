...
LOG_FILE="/var/spamtagger/log/spamtagger/updater4mc.log"
  ...
  if [[ $ABANDONED != '' ]]; then
    ...
    if grep -Fq "source" <<<"$ABANDONED"; then
      if grep -Fq "spamtagger" <<<"$ABANDONED"; then
        my_own_exit "1" # Abandoning update because Git tree at '/usr/spamtagger' is not on 'source' branch
      else
        my_own_exit "2" # Abandoning update because Git tree at '$rpath' is not on 'source' branch
      fi
    else
      if grep -Fq "spamtagger" <<<"$ABANDONED"; then
        my_own_exit "3" # Abandoning update because Git tree at '/usr/spamtagger' is blocking changes
      else
