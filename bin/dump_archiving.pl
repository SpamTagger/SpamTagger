...
sub dump_archived_domains()
{
  ...
  my $archive_path = "/var/spamtagger/spool/tmp/exim_stage1/archiver";
  if (! -d $archive_path) {
    mkdir($archive_path);
  }
...
sub dump_copyto()
{
  ...
  my $copyto_path = "/var/spamtagger/spool/tmp/exim_stage1/copyto";
  if (! -d $copyto_path) {
    mkdir($copyto_path);
  }
...
{
  my $bypassfiltering_path = "/var/spamtagger/spool/tmp/exim_stage1/bypass";
