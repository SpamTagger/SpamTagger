...
foreach (qw | to_domain to_user is_newsletter forced |) {
  $QUERY .= " $_ = '$args{$_}' AND" if (defined $args{$_});
}
...
printf("%-$_->{length}s ", $_->{heading}) foreach (@table);
print "\n";
print '-' foreach (1..80);
print "\n";

foreach my $msg (@messages) {
  $msg->{M_subject} = substr($msg->{M_subject},1) if ($msg->{M_subject} =~ m/^\ ?=\?/);
  printf("%-$_->{length}s ", substr($msg->{$_->{name}},0,$_->{length})) foreach (@table);
  print "\n";
}
