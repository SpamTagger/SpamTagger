...
#
#   This script will add a wantlist to the database.
#   This script is intended to be used by the SpamTagger SOAP API (Soaper).
#
#   Usage:
#     add_to_wantlist.pl msg_dest msg_sender
...
my $sth = $dbh->prepare("SELECT * FROM wwlists WHERE sender = ? AND recipient = ? AND type = 'want'") || err("CANNOTSELECTDB");
...
$sth = $dbh->prepare("INSERT INTO wwlists (sender, recipient, type, expiracy, status, comments)
  values (?, ?, 'want', '0000-00-00', 1, '[Wantlist]')");
$sth->execute($sender, $dest);
