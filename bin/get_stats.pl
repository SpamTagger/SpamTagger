...
if ($begin =~ /^\d{8}/ && $end =~ /^\d{8}/) {
  if (int($end) lt int($begin)) {
    bad_usage("end date should come after begin date ($begin, $end)", $begin, $end);
  }
  $start = $begin;
  $stop = $end;
}
if ($begin =~ /^(\d{4})(\d{2})(\d{2})$/ && $end =~ /^\+/) {
  $start = $begin;
  $stop = add_date($start, $end);
}
if ($end =~ /^(\d{4})(\d{2})(\d{2})$/ && $begin =~ /^\-/) {
  $stop = $end;
  $start = add_date($stop, $begin);
}
if ($begin =~ /^\-/ && $end =~ /^\+/) {
  $start = add_date($start, $begin);
  $stop = add_date($start, $end);
}
...
if (scalar(@dirs)) {
  foreach my $dir (@dirs) {
    ...
    while ($tday <= $stop) {
      ...
      return_stats() if $fulldays;
      $tday = add_date($tday, '+1');
      clear_stats() if $fulldays;
    }
    print $whats{$dir}.":" if $batchmode && !$fulldays;
    add_global_stats();
    return_stats() if !$fulldays;
...
sub process_file ($file) {
  my ($cmsgs, $cspams, $chighspams, $cviruses, $cnames, $cothers, $ccleans, $cbytes, $cusers, $cdomains) = Stats::read_file($file);
...
#######################
sub bad_usage ($bad, @errors) {
  print "Bad Usage: wrong paremeter: $bad (".join(', ',@errors).")\n";
